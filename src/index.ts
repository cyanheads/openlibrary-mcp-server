...
await createApp({
  ...
  instructions:
    'Use the openlibrary_* tools to search books, editions, authors, and subjects via Open Library. No API key required. IDs are typed by suffix — works OL…W, editions OL…M, authors OL…A — and editions also resolve by ISBN. Canonical flow: openlibrary_search_books → openlibrary_get_work → openlibrary_get_editions/openlibrary_get_edition; for people, openlibrary_search_authors → openlibrary_get_author. The work tools take only work IDs (OL…W): from an ISBN, openlibrary_get_edition with id_type "isbn" returns the parent work_id. openlibrary_get_edition takes a batch of identifiers of one type in a single call. To find a book from a passage rather than its metadata, use openlibrary_search_inside — it searches the scanned text of Internet Archive books and usually takes 10–30 seconds, so reach for it deliberately. Resolve cover/photo IDs with openlibrary_get_cover_url — it always returns HTTP 200, so a missing image is a 1×1 placeholder GIF, not an error.',
  // No tool calls ctx.requestInput, so nothing here needs a session to answer a
  // mid-handler prompt. Declared rather than left to MCP_SESSION_MODE's `auto`
